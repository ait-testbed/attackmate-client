    ...
    def _login(self, username: str, password: str) -> Optional[str]:
        ...
        try:
            ...
            if token:
                ...
                logger.error(f"Login to {self.server_url} succeeded but no token received (missing 'access_token').")
                return None
        except httpx.HTTPStatusError as e:
            logger.error(f"Login failed for '{username}' at {self.server_url}: {e.response.status_code} - {e.response.text}")
            return None
